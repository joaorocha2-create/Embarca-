# Embarcaê
## Descrição
O projeto busca resolver a falta de transporte coletivo em Iporá, atrvés do protótipo de aplicativo de transporte público com rotas que integram a maioria dos bairros com o centro urbano, de forma que atenda a grande parte dos morradores que possuem dificuldade de se deslocarem pelo município.
## Grupo
- Ana Julia Pequeno da Silva
- João Lucas Rocha de Sousa Pereira
- Laine Rodrigues Barros
- Mariana Silva Ferreira
## Protótipo no Figma
https://www.figma.com/design/aCCQAmYDJ7880pXNGSoplw/Prot%C3%B3tipo-----Transporte-p%C3%BAblico?node-id=0-1&t=5GxlCwSXFBLrVZvk-1
## Uso de Inteligência Artificial
Consulte o arquivo [USO_IA.md](./USO_IA.md) para ver como (e se) utilizamos IA neste projeto.
## Disciplina
IAUC — Introdução ao Ambiente Universitário e Computação
IF Goiano — Campus Iporá — 2026/1
